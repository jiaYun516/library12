from user_borrowAndReturn.models import Member, Book, Category
...
def homepage(request):
    ...
    for book in books:
        ...
        result += f"<a href='/book/{book.id}'>{book.title}</a><p>類型:<a href='/category/{book.category_id}'>{book.category}</a></p><p>作者:{book.author}</p>借閱量：{borrow_count}</p><br>"
...
def searchByCategory(request, category_id):
    try:
        ...
        if books:
            ...
            for book in books:
                borrow_count = book.borrowingrecord_set.count()
                result += f"<p>{book.title}</p> <p>作者:{book.author}</p> <p>館內餘量:{book.available_quantity}   借閱量：{borrow_count}</p><br>"
            return HttpResponse(result)
